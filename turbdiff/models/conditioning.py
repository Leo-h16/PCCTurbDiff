...
class Conditioning(nn.Module):
    ...
    def __init__(
        ...
        cell_pos: bool,
        mesh_encoder: nn.Module | None = None,
        mesh_embedding_dim: int = 0,
    ):
        super().__init__()
        ...
        self.mesh_embedding_dim_val = mesh_embedding_dim
    ...
    def forward(self, data: OpenFOAMData):
        ...
        if self.mesh_encoder:
            if hasattr(data, 'obstacle_mesh'):
                mesh_data = data.obstacle_mesh
                mesh_emb = self.mesh_encoder(**mesh_data)
            batch_size = data.n_samples
            if mesh_emb.shape[0] == 1 and batch_size > 1:
                    mesh_emb = mesh_emb.expand(batch_size, -1)
            C[Conditioning.Type.GEOMETRY] = mesh_emb
    ...
    def global_conditioning_dim(self):
        ...
        if self.mesh_encoder is not None:
            dim += self.mesh_embedding_dim_val
        return dim
...
def global_conditioning(C: dict[Conditioning.Type, torch.Tensor]) -> torch.Tensor | None:
    ...
    if len(c_global) == 0:
        ...
        return torch.cat(c_global, dim=1)
