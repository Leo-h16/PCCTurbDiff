...
from .obstacle import SingleObstacleData
...
class OpenFOAMData:
    ...
    def grid_embedding(self, variables: tuple[Variable, ...]):
        ...
        x = torch.zeros(
            (*batch_dims, sum(dims), *self.cell_counts),
            device=self.device
        )
        ...
        cell_idx = self.cell_idx.to(self.device)
        cell_type = self.cell_type.to(self.device)
        for v in variables:
            B = x_v[v].shape[0]
            C = x_v[v].shape[-1]

            src = self.samples[v].to(self.device)

            if x_v[v].dim() == 3 and src.dim() == 2:
                src = src.unsqueeze(0).expand(B, -1, -1)
            elif x_v[v].dim() == 2 and src.dim() == 3:
                src = src.squeeze(0)

            x_v[v].index_copy_(-2, cell_idx, src)

            idx = cell_idx[cell_type == 1]
            if len(idx) > 0:
                x_v[v].index_fill_(-2, idx, 0.0)

            if v == Variable.U:
                idx = cell_idx[cell_type == 4]
                if len(idx) > 0:
                    x_v[v].index_fill_(-2, idx, 0.0)

                idx = cell_idx[cell_type == 2]
                if len(idx) > 0:
                    K = len(idx)
                    inlet_shape = (*x_v[v].shape[:-2], K, C)
                    inlet_u = torch.zeros(inlet_shape, device=self.device)
                    inlet_u[..., 2] = 20.0
                    x_v[v].index_copy_(-2, idx, inlet_u)

                idx = cell_idx[cell_type == 3]
                if len(idx) > 0:
                    x_v[v].index_fill_(-2, idx, 0.0)
                ...
                idx = cell_idx[cell_type == 2]
                if len(idx) > 0:
                    x_v[v].index_fill_(-2, idx, 2e-05)

                idx = cell_idx[cell_type == 4]
                if len(idx) > 0:
                    x_v[v].index_fill_(-2, idx, 0.01)
...
class OpenFOAMDataRepository:
    ...
    def read_metadata(self, file_idx: int):
        ...
        with h5.File(self.files[file_idx], mode="r") as f:
            ...
            cell_type = torch.tensor(np.array(f["grid/cell_type"]))

            case_dir = file_path.parent
            case_name = case_dir.name
            stl_path = case_dir / f"{case_name}.stl"
        
            obstacle_mesh_data = None
        if stl_path.exists():
            loader = SingleObstacleData(stl_path, k_eig=64)
            obstacle_mesh_data = loader[0]
        ...
        return OpenFOAMMetadata(
            ...
            cell_idx=cell_idx,
            cell_type=cell_type,
            obstacle_mesh=obstacle_mesh_data,
        )
