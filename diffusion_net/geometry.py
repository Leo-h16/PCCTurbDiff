...
def chamfer_distance_loss(pred_points, target_points):
    ...
    min_dist_p_to_t, _ = torch.min(dist_matrix, dim=2)
    min_dist_t_to_p, _ = torch.min(dist_matrix, dim=1)
    
    return torch.mean(min_dist_p_to_t) + torch.mean(min_dist_t_to_p)


def repulsion_loss(pred_points, n_neighbors=5, radius=0.03):
    dist_matrix = torch.cdist(pred_points, pred_points, p=2)

    dist, _ = torch.topk(dist_matrix, k=n_neighbors + 1, dim=-1, largest=False)
    dist = dist[:, :, 1:] 
    

    loss = -torch.log(dist / radius + 1e-7) 
    loss = torch.clamp(loss, min=0) 
    
    return loss.mean()

def normal_consistency_loss(pred_points, gt_points, gt_normals):
    dist = torch.cdist(pred_points, gt_points)
    idx = torch.argmin(dist, dim=2) # [B, N]
    

    gathered_normals = torch.gather(gt_normals, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
    

    diff = pred_points - torch.gather(gt_points, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
    diff = F.normalize(diff, p=2, dim=-1)
    
    cos_sim = torch.abs(torch.sum(diff * gathered_normals, dim=-1))
    return 1.0 - cos_sim.mean()

def point_to_plane_loss(pred, gt, gt_normals):
    dist_sq = torch.cdist(pred, gt, p=2).pow(2)
    idx = torch.argmin(dist_sq, dim=2)
    
    idx_ext = idx.unsqueeze(-1).expand(-1, -1, 3)
    closest_gt = torch.gather(gt, 1, idx_ext)
    closest_normals = torch.gather(gt_normals, 1, idx_ext)
    dist_projection = torch.abs(torch.sum((pred - closest_gt) * closest_normals, dim=-1))
    return dist_projection.mean()